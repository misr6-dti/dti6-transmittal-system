...
class UpdateTransmittalRequest extends FormRequest
{
    /**
    ...
    {
        // Policy check handles this in controller
        return true;
    }
    ...
    {
        $id = $this->route('transmittal')->id;
        $isDraft = $this->input('status') === 'Draft';

        $rules = [
            'reference_number' => ['required', Rule::unique('transmittals')->ignore($id)],
