                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-1 text-sm font-medium text-slate-500 md:ml-2">Create</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h2 class="text-2xl font-extrabold text-navy">New Document Log</h2>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-200">
    <div class="p-6 md:p-8">
        <form action="{{ route('document-logs.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="reference_number" value="{{ old('reference_number', $refNumber) }}" class="w-full px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 cursor-not-allowed font-mono" readonly>
                </div>

                <!-- Date -->
                <div class="space-y-2">
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">Log Date</label>
                    <input type="date" name="log_date" value="{{ old('log_date', date('Y-m-d')) }}" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-navy/20 focus:border-navy text-sm transition-all @error('log_date') border-red-500 @enderror" required>
                    @error('log_date')
                ...
                <div class="space-y-2">
                    ...
                    <select name="receiver_division_id" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-navy/20 focus:border-navy text-sm transition-all @error('receiver_division_id') border-red-500 @enderror" required>
                        ...
                        @foreach($divisions as $division)
                            <option value="{{ $division->id }}" {{ old('receiver_division_id') == $division->id ? 'selected' : '' }}>{{ $division->name }} ({{ $division->code }})</option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="remarks" value="{{ old('remarks') }}" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-navy/20 focus:border-navy text-sm transition-all" placeholder="Any additional notes...">
                </div>
            </div>
...
<script>
    ...
    function addItem() {
        ...
        const html = template.replace(/INDEX/g, itemCount++);
