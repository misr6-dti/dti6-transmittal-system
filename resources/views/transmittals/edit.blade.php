    ...
    <div class="mb-8 border-b border-slate-200 pb-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center">
                <div class="bg-amber-50 p-3 rounded-xl text-amber-500 mr-4">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                </div>
                <div>
                    <h2 class="text-2xl font-extrabold text-navy">Update Transmittal Form</h2>
                    <p class="text-slate-500 text-sm">Modifying record #{{ $transmittal->reference_number }} for re-submission.</p>
                </div>
            </div>
            <div>
                <a href="{{ route('transmittals.show', $transmittal) }}" class="px-5 py-2.5 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 font-medium transition-colors">
                    Cancel Edits
                </a>
            </div>
        </div>
    ...
    <form action="{{ route('transmittals.update', $transmittal) }}" method="POST" x-data="transmittalForm()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 h-full">
                ...
                <div class="p-6">
                    <div class="mb-5">
                        ...
                        <input type="text" name="reference_number" value="{{ old('reference_number', $transmittal->reference_number) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-navy/20 focus:border-navy font-bold text-navy transition-all @error('reference_number') border-red-500 @enderror" required>
                    </div>
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Transmittal Date</label>
                        <input type="date" name="transmittal_date" value="{{ old('transmittal_date', $transmittal->transmittal_date ? $transmittal->transmittal_date->format('Y-m-d') : '') }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-navy/20 focus:border-navy text-slate-600 transition-all" required>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 h-full">
                ...
                <div class="p-6">
                    <div class="mb-5">
                        ...
                        <input type="text" value="{{ $transmittal->senderOffice->name }}" class="w-full px-4 py-2.5 bg-slate-100 border border-slate-200 rounded-xl text-slate-500 cursor-not-allowed" disabled>
                    </div>
                    <div>
                        ...
                        <select name="receiver_office_id" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-navy/20 focus:border-navy text-slate-600 transition-all appearance-none" required>
                            @foreach($offices as $office)
                                <option value="{{ $office->id }}" {{ old('receiver_office_id', $transmittal->receiver_office_id) == $office->id ? 'selected' : '' }}>{!! $office->display_name !!}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 h-full">
                ...
                <div class="p-6">
                    ...
                    <textarea name="remarks" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-navy/20 focus:border-navy text-slate-600 transition-all" rows="4" placeholder="Optional context...">{{ old('remarks', $transmittal->remarks) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 mb-8 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <div class="text-xs text-slate-500 font-medium bg-white px-2 py-1 rounded-md border border-slate-200">At least one item required</div>
            </div>
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-4 py-2">
                                    <input type="number" :name="'items['+index+'][quantity]'" x-model="item.quantity"
                                        class="w-full bg-transparent border-0 text-center font-bold focus:ring-0 p-0 text-navy placeholder-slate-300" placeholder="0">
                                </td>
                                <td class="px-4 py-2">
                                    <input type="text" :name="'items['+index+'][unit]'" x-model="item.unit"
                                        class="w-full bg-transparent border-0 focus:ring-0 p-0 text-slate-600 placeholder-slate-300" placeholder="pcs">
                                </td>
                                <td class="px-4 py-2">
                                    <textarea :name="'items['+index+'][description]'" x-model="item.description"
                                        rows="1" class="w-full bg-transparent border-0 focus:ring-0 p-0 text-slate-600 placeholder-slate-300 resize-none" placeholder="Document code..."></textarea>
                                </td>
                                <td class="px-4 py-2">
                                    <input type="text" :name="'items['+index+'][remarks]'" x-model="item.remarks"
                                        class="w-full bg-transparent border-0 focus:ring-0 p-0 text-slate-500 placeholder-slate-300 italic" placeholder="Optional...">
                                </td>
                                <td class="px-4 py-2 text-center">
                                    <button type="button" @click="removeItem(index)"
                                        class="text-red-400 hover:text-red-600 transition-colors" x-show="items.length > 1">
        ...
        <div class="flex justify-end gap-3 mb-8">
            <a href="{{ route('transmittals.show', $transmittal) }}"
                class="px-6 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50 font-bold transition-colors">Discard Changes</a>
            @if($transmittal->status === 'Draft')
                ...
                <button type="submit" name="status" value="Submitted"
                    class="px-8 py-3 bg-navy text-white rounded-xl hover:bg-navy-light font-bold shadow-lg shadow-navy/20 transition-all transform hover:-translate-y-0.5 flex items-center">
