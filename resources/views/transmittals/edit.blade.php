...
<div class="mb-5 border-bottom pb-4">
    <div class="row align-items-center">
        ...
        <div class="col">
            ...
            <p class="text-muted small mb-0">Modifying record #{{ $transmittal->reference_number }} for re-submission.</p>
        </div>
        <div class="col-auto">
            <a href="{{ route('transmittals.show', $transmittal) }}" class="btn btn-light rounded-pill px-4">Cancel Edits</a>
        </div>
    </div>
</div>
...
<form action="{{ route('transmittals.update', $transmittal) }}" method="POST" x-data="transmittalForm()">
            ...
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body">
                    <div class="mb-4">
                        ...
                        <input type="text" name="reference_number" value="{{ old('reference_number', $transmittal->reference_number) }}" class="form-control fw-bold" required>
                    </div>
                    <div>
                        <label class="form-label small fw-bold text-uppercase text-muted">Transmittal Date</label>
                        <input type="date" name="transmittal_date" value="{{ old('transmittal_date', $transmittal->transmittal_date) }}" class="form-control" required>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body">
                    <div class="mb-4">
                        ...
                        <input type="text" value="{{ $transmittal->senderOffice->name }}" class="form-control bg-light" disabled>
                    </div>
                    <div>
                        ...
                        <select name="receiver_office_id" class="form-select @error('receiver_office_id') is-invalid @enderror" required>
                            @foreach($offices as $office)
                                <option value="{{ $office->id }}" {{ old('receiver_office_id', $transmittal->receiver_office_id) == $office->id ? 'selected' : '' }}>{{ $office->name }}</option>
                            @endforeach
                        </select>
        ...
        <div class="col-md-4">
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body">
                    ...
                    <textarea name="remarks" class="form-control" rows="4" placeholder="Optional context...">{{ old('remarks', $transmittal->remarks) }}</textarea>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <tbody>
                        <template x-for="(item, index) in items" :key="index">
                            <tr>
                                ...
                                <td>
                                    <input type="number" :name="'items['+index+'][quantity]'" x-model="item.quantity" class="text-center fw-bold" placeholder="0">
                                </td>
                                <td>
                                    <input type="text" :name="'items['+index+'][unit]'" x-model="item.unit" placeholder="pcs">
                                </td>
                                <td>
                                    <textarea :name="'items['+index+'][description]'" x-model="item.description" rows="1" class="py-2" placeholder="Document code..."></textarea>
                                </td>
                                <td>
                                    <input type="text" :name="'items['+index+'][remarks]'" x-model="item.remarks" placeholder="Optional...">
                                </td>
    ...
    <div class="d-flex justify-content-end gap-3 mb-5">
        <a href="{{ route('transmittals.show', $transmittal) }}" class="btn btn-light rounded-pill px-4 fw-bold d-flex align-items-center">Discard Changes</a>
...
<style>
    #items-table tr td { vertical-align: middle; }
